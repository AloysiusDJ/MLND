...
def feature_plot(importances, X_train, y_train):
    ...
    values = importances[indices][:9]

    # Creat the plot
    fig = pl.figure(figsize = (13, 5))
    pl.title("Normalized Weights for First Five Most Predictive Features", fontsize = 16)
    pl.bar(np.arange(9), values, width = 0.6, align="center", color = '#00A000', \
          label = "Feature Weight")
    pl.bar(np.arange(9) - 0.3, np.cumsum(values), width = 0.2, align = "center", color = '#00A0A0', \
          label = "Cumulative Feature Weight")
    pl.xticks(np.arange(9), columns)
    pl.xlim((-0.5, 8.5))
